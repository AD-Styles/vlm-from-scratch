...
| **결과 요약** | 영문 VQA **4/5** ✅ · 한국어 ⚠️ catastrophic forgetting · OOD ⚠️ Hallucination ([상세](#-결과-results)) |
...
> 💭 왜 캡션 패턴만 모방하나? Stage 1 은 image embedding 을 LLM 공간에 "정렬" 만 하고 instruction-following 학습은 안 하기 때문. → Stage 2 가 필요한 이유!
...
원인: 학습 데이터 100% 영어 → LoRA가 영어 시각-언어 매핑만 강화 → base Qwen2.5의 한국어 능력이 부분 손상. **PEFT 사용 시 다국어 균형 데이터의 중요성**을 보여주는 결과. **→ v3 의 한국어 instruction 데이터 30%+ 추가 motivation** ([회고록 §Step 6 참조](#-회고록-retrospective)).
...
> 이 프로젝트는 **단발성 결과물이 아니라 6단계 의사결정 사이클** 의 기록입니다. 각 단계에서 어떤 한계를 발견하고, 어떤 옵션을 검토했고, 왜 그 선택을 했는지 정리했습니다.
